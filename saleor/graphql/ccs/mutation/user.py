...
from ...ccs.types import UserType
...
class UserCreateNew(graphene.Mutation):
    userObj = graphene.Field(UserType)
    ...
    class Arguments:
        ...
        issuper = graphene.Boolean(required=True)
    ...
    def mutate(cls, root, info, **data):
        user = models.User.objects.create(user_id=data['email'], email=data['email'], is_superuser=data['issuper'])
        user.set_password(data["password"])
        user.save
        return UserCreateNew(userObj=user)
